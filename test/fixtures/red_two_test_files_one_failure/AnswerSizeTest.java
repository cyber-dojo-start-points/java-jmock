...
public class AnswerSizeTest {
    ...
    public void the_answer_is_two_digits_long() throws Exception {
        context.checking(new Expectations() {{
            oneOf(answerSeeker).receiveAnswer(with(allOf(greaterThan(9), lessThan(100))));
        }});
        deepThought.computeForSevenMillionYears();
    }

    @Test
    public void the_answer_is_three_digits_long() throws Exception {
        context.checking(new Expectations() {{
            oneOf(answerSeeker).receiveAnswer(with(greaterThan(99)));
        }});
        deepThought.computeForSevenMillionYears();
    }
}
